...
form.addEventListener('submit', function (e) {
	e.preventDefault();
	//even though the form type is number it always return the value as string
	const value = parseInt(amount.value);
	const random = Math.floor(Math.random() * text.length);
	//empty value
	//negative value
	// large the the length of text
	if (isNaN(value) || value <= 0) {
		result.innerHTML = `<p class="result">Please place a valid number value</p>`;
		result.firstChild.style.color = 'red';
	} else {
